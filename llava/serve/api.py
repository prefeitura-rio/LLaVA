...
def get_args():
    parser = argparse.ArgumentParser(description="LLaVA Flask API")

    parser.add_argument(
        "-p", "--port", help="Port to listen on", type=int, default=5000
    )
...
def generate_wrapper(
    input_ids, image_tensor, temperature, max_new_tokens, streamer, stopping_criteria
):
    ...
    if data["conv_mode"] is not None and conv_mode != data["conv_mode"]:
        print(
            "[WARNING] the auto inferred conversation mode is {}, while `--conv-mode` is {}, using {}".format(
                conv_mode, data["conv_mode"], data["conv_mode"]
            )
        )
    else:
...
def not_found(error):
    return make_response(
        jsonify(
            {"status": "error", "msg": f"{request.url} not found", "detail": str(error)}
        ),
        404,
    )
...
def internal_server_error(error):
    return make_response(
        jsonify(
            {"status": "error", "msg": "Internal Server Error", "detail": str(error)}
        ),
        500,
    )
    ...
    try:
        ...
        if stream:
            ...
            outputs = run_inference(
                data, CURRENT_MODEL_PATH, tokenizer, model, image_processor, context_len
            )
                ...
                jsonify(
                    {
                        "status": "ok",
                        "response": "\n".join(
                            [output.replace("</s>", "") for output in outputs]
                        ),
                    }
                ),
                200,
